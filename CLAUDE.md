...
- **Fluxo atual de telas:** `SplashScreen` (toque para continuar) → `MenuScreen` → **JOGAR** → `CutsceneScreen` → (pushReplacement) `TutorialScreen` → **TODO: level 1** (por ora o fim do tutorial volta ao menu).
